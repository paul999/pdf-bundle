...
class PuppeteerPdfGenerator implements PdfGeneratorInterface
{
    ...
    private function makeUrlsLocal(string $html): string
    {
        ...
        return preg_replace($relativePattern, $relativeReplacement, $html);
    }
    ...
    private function renderInPuppeteer(string $filename, Format $format): PdfInterface
    {
        ...
        $browser->path = $browser->path . ';XDG_CONFIG_HOME="$WORKSPACE@tmp/.chromium";XDG_CACHE_HOME="$WORKSPACE@tmp/.chromium";PUPPETEER_CACHE_DIR=/srv/3dprintcalculator.net/puppeteer/.cache';
